...
from .mongo_serializers import ReservationsEventSerializer
...
def reservation_event_list_create(request):
    if request.method == "GET":
        q = dict(request.query_params)
        docs = [fix_id(d) for d in col.find(q)]
        return Response(docs)

    serializer = ReservationsEventSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def reservation_event_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = ReservationsEventSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
